...
async fn migration_011_rolls_back_and_reapplies(pool: PgPool) {
    // Dependent data blocks the down migration (no CASCADE). 014's sessions
    // table, 013's assignment columns and 012's index-only down run first,
    // then the probe must block 011's table drop.
    let f = Fixture::new(&pool).await;
    f.started().await;
    exec(
        &pool,
    ...
    MIGRATOR
        ...
        .await
        .unwrap_or_else(|error| panic!("013/012 downs must apply: {error}"));
